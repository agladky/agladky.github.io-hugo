...
Теперь, если запустить скрипт как указано выше, то при вызове `args.token` получим значение введенного токена. Про остальные параметры и методы можно узнать в [документации python](https://docs.python.org/3.5/library/argparse.html).
