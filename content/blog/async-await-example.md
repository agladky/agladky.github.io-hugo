...
Это краткая выжимка из рабочего доклада, который я готовил для объяснения работы с async/await в C#.  Для наглядности, параллельно рассматривается подход с использованием блока `ContinueWith`.
